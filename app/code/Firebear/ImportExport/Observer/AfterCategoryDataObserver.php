...
class AfterCategoryDataObserver implements ObserverInterface
{
    ...
    public function execute(Observer $observer)
    {
        ...
        foreach ($stores as $store) {
            $rootStoreCategories[$store->getCode()] = $store->getRootCategoryId();
        }
        if ($categories = $observer->getEvent()->getBunch()) {
            foreach ($categories as $category) {
                $categoryStoreView = $category['store_view'] ?? '';
                $categoryPathRow = $category['path'] ?? '';
                if (empty($categoryPathRow) || empty($categoryStoreView)) {
                    continue;
                }
                $categoryPath = explode('/', $categoryPathRow);
                if (empty($categoryPath) || empty($rootStoreCategories[$categoryStoreView])) {
                    continue;
                }
                $categoryStoreRootPath = $rootStoreCategories[$categoryStoreView];
                if (in_array($categoryStoreRootPath, $categoryPath)) {
    ...
    protected function _populateForUrlGeneration(array $rowData)
    {
        ...
        if (empty($this->urlPathStoreIdPairs[$storeId]) ||
            !in_array($category->getRequestPath(), $this->urlPathStoreIdPairs[$storeId])) {
            $this->urlPathStoreIdPairs[$storeId][] = $category->getRequestPath();
