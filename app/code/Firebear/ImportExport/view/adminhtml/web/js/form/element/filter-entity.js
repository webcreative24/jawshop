        ...
        return Acstract.extend(general).extend(
            {
                defaults: {
                    ...
                    imports: {
                        changeSourceByFilterType: '${$.parentName}.source_filter_type:value'
                    },
                    listens: {
                ...
                changeSource: function (value) {
                    ...
                    if (data !== null && typeof data === 'object') {
                        ...
                        if (selectOptions.length !== 0 && selectOptions.includes(oldValue)) {
                            self.value(oldValue);
                ...
                changeSourceByFilterType: function (value) {
                    let getType,
                        fromInputName = 'input[name="source_filter_map[from][source_filter_filter]"][aria-describedby="notice-' + this.uid +'"]',
                        toInputName = 'input[name="source_filter_map[to][source_filter_filter]"][aria-describedby="notice-' + this.uid +'"]',
                        fromInput = $(fromInputName),
                        toInput = $(toInputName),
                        fromInputDbName = 'input[name="source_filter_map[shifted][source_filter_filter]"][aria-describedby="notice-' + this.uid +'"]',
                        toInputDbName = 'input[name="source_filter_map[second_shifted][source_filter_filter]"][aria-describedby="notice-' + this.uid +'"]',
                        fromDbInput = $(fromInputDbName),
                        toDbInput = $(toInputDbName),
                        dataScope = this.dataScope,
                        parts = dataScope.split('.'),
                        recordNumber = parts[2],
                        filterField = this.ns + '.' + this.ns + '.source_data_filter_container.source_filter_map.' + recordNumber + '.source_filter_field',
                        attribute = reg.get(filterField);

                    if (typeof attribute !== 'undefined') {
                        this.changeSource(attribute.value());
                    }
                    getType = this.genType;

                    switch (value) {
                        case 'contains':
                        case 'not_contains':
                            this.changeTypes('text');
                            break;
                        case 'range_int':
                            this.changeTypes('int');
                            fromInput = $(fromInputName);
                            toInput = $(toInputName);
                        ...
                        case 'more_or_equal_date':
                            this.changeTypes('date');
                            fromDbInput = $(fromInputDbName);
                            toDbInput = $(toInputDbName);
                            toDbInput.prop('disabled', true);
                            fromDbInput.prop('disabled', false);
                            break;
                        case 'less_or_equal_date':
                            this.changeTypes('date');
                            toDbInput = $(toInputDbName);
                            fromDbInput = $(fromInputDbName);
                            fromDbInput.prop('disabled', true);
                            toDbInput.prop('disabled', false);
                            break;
                        case 'range_date':
                            this.changeTypes('date');
                            fromDbInput = $(fromInputDbName);
