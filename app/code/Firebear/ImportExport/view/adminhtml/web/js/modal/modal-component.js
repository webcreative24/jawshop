    ...
    return Parent.extend({
        ...
        setData:function (counter, count, error, job, file, step, object, status) {
            ...
            if (count <= counter - 1) {
                $.get(
                    ...
                    function (response) {
                        ...
                        if (response.result == true) {
                            ...
                            status = 0;
                            self.finish(false);
                            return self.terminateHandlers(job, file, status);
                        }
                    }
                ).fail(
                    function (response) {
                ...
                return this.terminateHandlers(job, file, status);
            }
        },
        terminateHandlers: function (job, file, status) {
            var jobStatus = status > 0;
            var  self = this;
