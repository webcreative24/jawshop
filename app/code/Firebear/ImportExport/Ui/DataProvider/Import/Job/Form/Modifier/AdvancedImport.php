    ...
    protected function addFieldSource()
    {
        ...
        foreach (array_merge_recursive($sourceConfig, $types) as $typeName => $type) {
            ...
            foreach ($type['fields'] as $name => $values) {
                ...
                if ((isset($values['type']) && $values['type'] == 'text')
                    && (isset($values['template']) && empty($values['template']))
                    && (isset($values['formElement']) && empty($values['formElement']))) {
                        $localConfig['elementTmpl'] = 'Firebear_ImportExport/form/element/input';
                }
                        ...
                        'arguments' => [
                            'data' => [
                                'config' => [
                                    'componentType' => 'field',
                                    'dataType' => 'text',
                                    'label' => __('Disable products by Supplier'),
                                    'formElement' => 'select',
                                    'source' => 'import',
