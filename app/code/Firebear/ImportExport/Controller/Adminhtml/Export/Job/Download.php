...
use Firebear\ImportExport\Model\LocalFilesystem;
        ...
        try {
            ...
            if (!empty($path)) {
                ...
                if ($this->isValid($path)) {
                    ...
                    if (!is_a($dir->getDriver(), File::class)) {
                        $fileContent = $dir->readFile($path);
                        $mediaDir = $this->localFilesystem->getDirectoryWrite(DirectoryList::MEDIA);
                        $filePath = $mediaDir->getAbsolutePath($fileName);
                        $mediaDir->writeFile($filePath, $fileContent);
