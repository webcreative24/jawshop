...
class Processor extends \Firebear\ImportExport\Model\AbstractProcessor
{
    ...
    const SOURCE_FILTER_TYPE = 'source_filter_type';
    ...
    const EXPORT_FILTER_TYPE = 'export_filter_type';
    ...
    const LAST_EXPORT_TYPE = 'last_export_type';
    ...
    public function getMapData($mapData)
    {
        ...
        foreach ($mapData as $key => $items) {
                ...
                foreach ($items as $num => $values) {
                    if ($num == self::VALUE) {
                        foreach ($values as $k => $field) {
                            ...
                            $exportFilterType[$k]['field'] = $field;
                            $exportFilterType[$k]['value'] = $parsedFilterValue;
                            $exportFilterTable[] = [
            ...
            if ($key == self::SOURCE_FILTER_TYPE) {
                foreach ($items as $num => $values) {
                    if ($num == self::VALUE) {
                        foreach ($values as $k => $filterType) {
                            $exportFilterType[$k]['type'] = $filterType;
                        }
                    }
                }
            }
    ...
    protected function getDataMerge()
    {
        ...
        if (isset($sourceData[self::SOURCE . '_entity'])) {
            ...
            $data = [
                ...
                self::EXPORT_FILTER_TYPE => $exportFilterType,
        ...
        if (isset($sourceData[self::LAST_EXPORT_TYPE])) {
            $data[self::LAST_EXPORT_TYPE] = $sourceData[self::LAST_EXPORT_TYPE];
        }
