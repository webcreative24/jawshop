    ...
    protected function prepareImagesFromSource($bunch)
    {
        foreach ($bunch as $rowNum => &$rowData) {
            ...
            foreach ($this->_imagesArrayKeys as $image) {
                ...
                if (preg_match('/\bhttps?:\/\//i', $rowData[$image])) {
                    continue;
                }
    ...
    public function createAttributeValues(
        ...
        if (isset($rowData[self::COL_ATTR_SET])) {
            ...
            foreach ($rowData as $attrCode => $attrValue) {
                ...
                if (empty($attrParams) && method_exists($productTypeModel, 'initAttributes')) {
                    $productTypeModel->initAttributes();
                    $attrParams = $productTypeModel->retrieveAttribute($attrCode, $attributeSet);
                }
                if (!empty($attrParams)) {
                ...
                foreach ($options as $attributeId => $optionsArray) {
                    foreach ($optionsArray as $option) {
                        ...
                        foreach ($this->_productTypeModels as $model) {
                            $model->addAttributeOption(
                                $option['code'],
                                strtolower($option['value']),
                                $intOptionId
                            );
                        }
                        $attributeIdsForCacheUpdating[$attributeId] = true;
                    }
                }
            }
        ...
        if (!empty($attributeIdsForCacheUpdating) && method_exists($productTypeModel, 'resetAttributeCache')) {
            $productTypeModel->resetAttributeCache(array_keys($attributeIdsForCacheUpdating));
        }
        return $rowData;
    }
