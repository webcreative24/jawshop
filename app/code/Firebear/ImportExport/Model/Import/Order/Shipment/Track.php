...
class Track extends AbstractAdapter
{
     ...
     */
    const COLUMN_SHIPMENT_SOURCE_CODE = 'source_code';
    ...
    const ERROR_SOURCE_ITEM_NOT_FOUND = 'sourceItemNotFound';
    ...
    public function __construct(
        ...
        if ($this->hasMsi() && $this->isEnableCoreMsiModules()) {
            $this->sourceItemRepository = ObjectManager::getInstance()->get(SourceItemRepositoryInterface::class);
        }
    }
        ...
        if (!empty($this->_currentOrderId)) {
            ...
            if (!empty($this->_parameters['generate_shipment_by_track']) &&
                ...
                if ($shipment->getTotalQty()) {
                    ...
                    if (!empty($rowData[self::COLUMN_SHIPMENT_SOURCE_CODE])) {
                        $extensionAttributes = $shipment->getExtensionAttributes();
                        $extensionAttributes->setSourceCode($rowData[self::COLUMN_SHIPMENT_SOURCE_CODE]);
    ...
    protected function _validateRowForUpdate(array $rowData, $rowNumber)
    {
        if (!empty($rowData[self::COLUMN_SHIPMENT_INCREMENT_ID])) {
            ...
            if ((empty($rowData[self::COLUMN_SHIPMENT_INCREMENT_ID]) && empty($rowData[self::COLUMN_SHIPMENT_ID])) &&
                empty($this->_parameters['generate_shipment_by_track']) &&
                1 < $shipmentCount) {
                ...
                foreach ($data as $sku => $qty) {
                    ...
                    if (!empty($rowData[self::COLUMN_SHIPMENT_SOURCE_CODE])) {
                        ...
                        $searchCriteria = $searchCriteriaBuilder
                            ->addFilter(SourceItemInterface::SOURCE_CODE, $rowData[self::COLUMN_SHIPMENT_SOURCE_CODE])
                        ...
                        if ($sourceItemsResult->getTotalCount() === 0) {
                            $this->addRowError(
                                self::ERROR_SOURCE_ITEM_NOT_FOUND,
                                $rowNumber,
                                $rowData[self::COLUMN_SHIPMENT_SOURCE_CODE],
                                __(
                                    'Source item not found by source code: %1 and sku: %2.',
                                    $rowData[self::COLUMN_SHIPMENT_SOURCE_CODE],
                                    $sku
                                )
                            );
