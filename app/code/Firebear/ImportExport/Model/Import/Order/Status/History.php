...
class History extends AbstractAdapter
{
    ...
    public function prepareRowData(array $rowData)
    {
        ...
        if (empty($rowData[self::COLUMN_ORDER_ID])
            && !empty($rowData['comment'])
            && !empty($rowData['entity_name'])) {
            $parentId = $this->getOrderEntityId($this->_currentOrderId);
            if ($parentId) {
                $rowData[self::COLUMN_ORDER_ID] = $parentId;
            }
        }
    ...
    protected function getOrderEntityId(string $incrementId): int
    {
        $tableName = $this->_resource->getTableName('sales_order');
        $select = $this->_connection->select();
        $select->from($tableName, 'entity_id')
    ...
    protected function _prepareDataForUpdate(array $rowData)
    {
        ...
        $rowData[self::COLUMN_ENTITY_ID] = !empty($rowData[self::COLUMN_ENTITY_ID])
            ? $rowData[self::COLUMN_ENTITY_ID] : (int) $entityId;
    ...
    {
        if (empty($rowData[self::COLUMN_ORDER_ID]) && empty(self::COLUMN_INCREMENT_ID)) {
