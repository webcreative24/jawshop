...
class AdvancedPricing extends MagentoAdvancedPricing
{
    ...
    public function deleteAdvancedPricing()
    {
        ...
        if ($tierPrices) {
            foreach ($tierPrices as $rowSku => $rowData) {
                foreach ($rowData as $row) {
                    $this->deleteSpecificTierPrice($rowSku, $row, self::TABLE_TIER_PRICE);
                }
            }
            $this->setUpdatedAt($listSku);
        }
     ...
     * @throws Exception
     */
    protected function deleteSpecificTierPrice($productSku, $data, $table)
    {
        $tableName = $this->_resourceFactory->create()->getTable($table);
        $productEntityLinkField = $this->getProductEntityLinkField();
        if ($tableName && !empty($productSku)) {
            ...
            if (!empty($productIds)) {
                try {
                    $condition = $this->_connection->quoteInto($productEntityLinkField . ' IN (?)', $productIds) .
                        ' AND ' . $this->_connection->quoteInto(
                            ' qty = ?',
                            number_format($data['qty'], 4, '.', '')
                        ).
                        ' AND ' . $this->_connection->quoteInto(' customer_group_id = ?', $data['customer_group_id']) .
                        ' AND ' . $this->_connection->quoteInto(TierPrice::WEBSITE_ID . ' IN (?)', $data['website_id']);
