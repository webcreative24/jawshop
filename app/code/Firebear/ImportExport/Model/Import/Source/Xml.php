...
class Xml extends AbstractSource
{
    ...
    public function __construct(
        ...
        if (!empty($data['predefined_structure']) && !empty($data['path_to_entity_structure'])) {
            $this->usePathToEntityStructure($data['path_to_entity_structure']);
        }
    ...
    /**
     * Use XPath to the parent element key to define the entity structure in the import file
     *
     * @param $xpath
     */
    protected function usePathToEntityStructure($xpath)
    {
        $xpathNode = $this->reader->xpath($xpath);
        if (!empty($xpathNode)) {
            $this->reader = array_shift($xpathNode);
        }
    }
