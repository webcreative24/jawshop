...
class Address extends MagentoAddress
{
     ...
     * @var string[]
     */
    protected $_ignoredAttributes = ['street'];
    ...
    public function customChangeData($rowData)
    {
        ...
        if (!isset($rowData['street']) || empty($rowData['street'])) {
            $street = [];
            for ($i = 1; $i <= 4; $i++) {
                $key = 'street' . $i;
    ...
    public function _prepareDataForUpdate(array $rowData):array
    {
        ...
        if ($updateData['entity_row_new'] && count($updateData['entity_row_new'])) {
            if (isset($rowData['_entity_id']) && $rowData['_entity_id'] > 0) {
    ...
    protected function _validateRowForUpdate(array $rowData, $rowNumber)
    {
        ...
        if (empty($rowData['street'])
            && empty($rowData['street1'])
            && empty($rowData['street2'])
            && empty($rowData['street3'])
            && empty($rowData['street4'])) {
            $this->addRowError(self::ERROR_VALUE_IS_REQUIRED, $rowNumber, 'street');
        }
    }
}
