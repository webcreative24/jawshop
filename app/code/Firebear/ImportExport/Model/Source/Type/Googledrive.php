...
class Googledrive extends AbstractType
{
    ...
    public function run($model)
    {
        ...
        try {
            ...
            $name = 'export_' . $this->timezone->date()->format('Y_m_d_H_i_s'). $info['basename'];
    ...
    {
        if (!$this->client) {
            if (!is_a($this->directory->getDriver(), File::class)) {
                $signingKeyFilePath = $this->getData('signing_key_file_path');
            } else {
    ...
    protected function _createFile($name, $mimeType, $parentFolderId = null, $filePath = null)
    {
        ...
        if ($filePath) {
            $params['data'] = $dir->readFile($filePath);
         ...
         */
        $qeury = "name='{$name}' and parents in '{$parentFolderId}'";
        $existingFile = $this->_getFile($qeury);
        if ($existingFile) {
