...
abstract class AbstractType extends DataObject implements SourceTypeInterface
{
    ...
    protected function writeFile($path)
    {
        ...
        if ($fileContent) {
            ...
            $lastPageExported = $this->cache->load('page_by_page_export_status');
            ...
            $result = $dir->writeFile($path . $fileName, $fileContent, "w");
            if ($result) {
                $data = $this->getExportModel()->getData(Processor::BEHAVIOR_DATA);
                if ((!empty($data['archive_after_export']) && $lastPageExported)
                    || (!empty($data['archive_after_export'] && empty($data['export_by_page'])))) {
