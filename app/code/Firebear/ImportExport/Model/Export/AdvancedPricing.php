...
class AdvancedPricing extends MagentoAdvancedPricing implements EntityInterface
{
    ...
    private function fetchTierPrices(array $productIds): array
    {
        ...
        if (isset($qty) && !empty($qty)) {
            $filterCondition = $this->getExportFilterType('tier_price_qty', $qty);
            $condition = ($filterCondition == 'not_contains') ? '!=' : '=';
            $select->where("ap.qty $condition ?", $qty);
        }
        if (isset($website) && !empty($website)) {
            $filterCondition = $this->getExportFilterType('tier_price_website', $website);
            $condition = ($filterCondition == 'not_contains') ? '!=' : '=';
            $select->where("ap.website_id $condition ?", trim($website));
        }
        if (isset($customer_group) && !empty($customer_group)) {
            $filterCondition = $this->getExportFilterType('tier_price_customer_group', $customer_group);
            $condition = ($filterCondition == 'not_contains') ? '!=' : '=';
            $select->where("ap.customer_group_id $condition ?", $customer_group);
        }
        ...
        if ($numeric !== null) {
            $filterCondition = $this->getExportFilterType('tier_price', $numeric);
            $condition = ($filterCondition == 'not_contains') ? '!=' : '=';
            if ($filterCondition == 'not_contains') {
                $select->where("ap.value $condition ?", $numeric);
                $select->where("ap.percentage_value $condition ?", $numeric);
                $select->orWhere('ap.percentage_value  IS NULL');
            } else {
                $select->where("ap.value $condition ?", $numeric);
                $select->orWhere("ap.percentage_value $condition ?", $numeric);
            }
        }
