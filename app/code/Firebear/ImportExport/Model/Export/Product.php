...
class Product extends \Magento\CatalogImportExport\Model\Export\Product implements EntityInterface
{
    ...
    public function _prepareEntityCollection(AbstractCollection $collection)
    {
            ...
            foreach ($this->filterAttributeCollection($this->getAttributeCollection()) as $attribute) {
                ...
                if (isset($exportFilter[$attrCode])) {
                    ...
                    if (Export::FILTER_TYPE_SELECT == $attrFilterType
                        ...
                        $filterCondition = $this->getExportFilterType($attrCode, $exportFilter[$attrCode]);
                        ...
                        if ($defaultValue === $exportFilter[$attrCode]) {
                            if ($condition == 'eq') {
                                ...
                                $collection->addAttributeToFilter($attrCode, ['neq' => (int)$exportFilter[$attrCode]]);
                            }
                        } else {
                            if ($attribute->getFrontendInput() == 'boolean'
                                ...
                                $collection
                                    ->addAttributeToFilter($attrCode, [$condition => (int)$exportFilter[$attrCode]]);
                            }
                        }
        ...
        $collection = $this->filterEntityCollection($collection);
        ...
        if (isset($exportFilter[$attrCode])) {
            ...
            $collection->getSelect()
                ...
                ->where("is_in_stock $condition", $inStock);
        }
     ...
     * @throws LocalizedException
     */
    protected function filterEntityCollection($collection)
    {
            ...
            if (isset($exportFilter[$attrCode])) {
                ...
                if (\Magento\ImportExport\Model\Export::FILTER_TYPE_SELECT == $attrFilterType) {
                    if (is_scalar($exportFilter[$attrCode]) && trim($exportFilter[$attrCode])) {
                        $condition = ($filterCondition == 'not_equal') ? 'neq' : 'eq';
                        $collection->addAttributeToFilter($attrCode, [$condition => $exportFilter[$attrCode]]);
                    }
                    ...
                    if (is_array($exportFilter[$attrCode])) {
                        ...
                        if (!empty($exportFilter[$attrCode])) {
                            $condition = ($filterCondition == 'not_equal') ? 'nfinset' : 'finset';
                    ...
                    if (is_scalar($exportFilter[$attrCode]) && trim($exportFilter[$attrCode])) {
                        ...
                        if ($condition == 'nlike') {
                            ...
                            $collection
                                ->addAttributeToFilter($attrCode, [$condition => "%{$exportFilter[$attrCode]}%"]);
                        }
                    }
                    ...
                    if (is_array($exportFilter[$attrCode]) && count($exportFilter[$attrCode]) == 2) {
                        switch ($filterCondition) {
                            case 'less_or_equal_date':
                                array_shift($exportFilter[$attrCode]);
                    ...
                    if (is_array($exportFilter[$attrCode]) && count($exportFilter[$attrCode]) == 2) {
                        switch ($filterCondition) {
                            case 'equal':
                                $from = array_shift($exportFilter[$attrCode]);
                                $collection->addAttributeToFilter($attrCode, ['eq' => $from]);
                                break;
                            case 'not_equal':
                                $from = array_shift($exportFilter[$attrCode]);
                                $collection->addAttributeToFilter($attrCode, ['neq' => $from]);
                                break;
                            case 'more_or_equal':
                                $from = array_shift($exportFilter[$attrCode]);
                                if (!empty($from)
    ...
    protected function addAdditionalFields(array $rows): array
    {
        foreach ($this->additionalFieldsPool->getEntities() as $keyEntity => $entity) {
            if ($entity instanceof Msi
                && isset($this->_parameters[Processor::BEHAVIOR_DATA])
                && empty($this->_parameters[Processor::BEHAVIOR_DATA]['import_msi'])
            ) {
                continue;
            }
