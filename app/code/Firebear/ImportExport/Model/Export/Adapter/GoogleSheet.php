...
class GoogleSheet extends AbstractAdapter
{
    ...
    public function writeRow(array $rowData)
    {
        ...
        if ($this->_data['behavior_data']['export_by_page'] == 1
            && $this->getLastHistoryType() == 'admin') {
            $page = $this->_cache->load('current_page');

            if ($this->_headerCols === null) {
                $this->_headerCols = array_keys($rowData);
            }

            if ($this->_headerCols === null && $page == 1) {
        ...
        $resultData = [];
        $headers = array_values($this->_headerCols);
        foreach ($headers as $key) {
            if (isset($rowData[$key])) {
                $resultData[$key] = $rowData[$key];
            } else {
                $resultData[$key] = '';
            }
        }

        $this->_exportQueue[] = array_values($resultData);

        if ($this->getBatchSize()
            && count($this->_exportQueue)
            && count($this->_exportQueue) >= $this->getBatchSize()
        ) {
            $this->runExportQueue();
        }
    ...
    public function setHeaderCols(array $headerColumns)
    {
        ...
        if ($headerColumns) {
            foreach ($headerColumns as $columnName) {
                $this->_headerCols[] = $columnName;
            }
        }
        $this->_exportQueue[] = array_values($this->_headerCols);
        return $this;
    }
    ...
    protected function exportQueue()
    {
        if ($this->_data['behavior_data']['export_by_page'] == 1
            ...
            if (($this->_linesCounter == 1) && ($page == 1)) {
                $this->clearTable();
                $this->prepareTableColumns(count($this->_headerCols));
            } elseif ($page > 1) {
                $this->_linesCounter = $this->_currentRowsCount;
            }
        } else {
